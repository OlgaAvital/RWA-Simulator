...
4. בדוק שאין כפילויות בין `App.jsx` לשכבת ה-domain:
...
6. פתח את הכתובת שמופיעה במסוף, בדרך כלל:
...
- בכל Pull Request רץ `npm run check:duplicates`, נבנה `dist` ומועלה artifact בשם `rwa-simulator-dist-preview` לצורך בדיקת תוצר ה־preview.
